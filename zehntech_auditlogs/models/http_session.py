...
class AuditlogtHTTPSession(models.Model):
    ...
    display_name = fields.Char("Name", compute="_compute_display_name", store=True, help="This is the display name for the HTTP user session log.")
    name = fields.Char("Session ID", index=True, required=True, help="The unique identifier for the HTTP session.")
    user_id = fields.Many2one("res.users", string="User", index=True, required=True, help="The user associated with session.")
    http_request_ids = fields.One2many(
        "auditlog.http.request", "http_session_id", string="HTTP Requests",  help="The HTTP requests associated with session."
    )
