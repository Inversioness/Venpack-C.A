...
class AuditlogHTTPRequest(models.Model):
    ...
    display_name = fields.Char("Name", compute="_compute_display_name", store=True, help="This is the display name of the HTTP request log.")
    name = fields.Char("Path", required=True, help="The path of the HTTP request.")
    root_url = fields.Char("Root URL", required=True, help="The root URL of the HTTP request.")
    user_id = fields.Many2one("res.users", string="User", help="The user who made the HTTP request.")
    http_session_id = fields.Many2one("auditlog.http.session", string="Session", index=True,  help="The HTTP session associated with this request.")
    user_context = fields.Char("Context", required=True,  help="The context or details related to the user session for this request.")
    log_ids = fields.One2many("auditlog.log", "http_request_id", string="Logs", required=True, help="The logs related to this HTTP request.")
