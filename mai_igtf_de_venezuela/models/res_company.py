...
class ResCompany(models.Model):
	...
	receivable_account_id = fields.Many2one('account.account', string='Cuenta Recibos IGTF', 
		domain=[('reconcile', '=', True)], ondelete='restrict')
...
class AccountPayment(models.Model):
	...
	def _prepare_move_line_default_vals_igft(self, write_off_line_vals=None):
		...
		if not self.receivable_account_id or not self.payable_account_id:
			raise exceptions.UserError(_("You can't create a new payment without an outstanding payments/receipts account set in company."))
		...
		outstnding_acc = self.outstanding_account_id.id

		available_payment_method_lines = self.igtf_journal_id._get_available_payment_method_lines(self.payment_type)
		if available_payment_method_lines :
			payment_method_line_id = available_payment_method_lines[0]._origin
			if payment_method_line_id :
				if self.payment_type == 'inbound':
					outstnding_acc = (payment_method_line_id.payment_account_id  or  self.igtf_journal_id.company_id.account_journal_payment_debit_account_id).id
				elif self.payment_type == 'outbound':
					outstnding_acc = (payment_method_line_id.payment_account_id or  self.igtf_journal_id.company_id.account_journal_payment_credit_account_id).id
		
		if self.payment_type == 'inbound':
			desti_acc = self.receivable_account_id.id
		elif self.payment_type == 'outbound':
			desti_acc = self.payable_account_id.id
		else:
			desti_acc = self.destination_account_id.id
